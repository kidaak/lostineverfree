	...
	$('form#mpcth_contact_form').submit(function() {
		...
		if(!hasError) {
			...
			$.post($this.attr('action'), formInput, function(data) {
				if(data == 'success') {
					$this.find('#message_cf').after('<p class="mpcth-cf-response mpcth-cf-success">' + cfdata.success_msg + '</p>');
					...
					$this.find('#message_cf').after('<p class="mpcth-cf-response mpcth-cf-error">' + cfdata.error_msg + '</p>');
				}
			});
		} 
